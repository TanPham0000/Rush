<script lang="ts">
  ...
  const CODEX = [
    { name:'INFANTRY',     role:'ASSAULT',    roles:['ANTI-INFANTRY','GARRISON'],
      hp:100, spd:65,  atk:15,  rng:55,  cost:100,
      color:'#44AAFF', shape:'circle',
      lore:'Backbone of any force. Cheap, versatile, and effective in cover. Suppresses enemies and holds objectives. 8s to train.',
    },
    { name:'GRENADIER',    role:'SPLASH',     roles:['SPLASH DMG','ANTI-INFANTRY'],
      hp:110, spd:58,  atk:28,  rng:70,  cost:100,
      color:'#FF8800', shape:'circle',
      lore:'Replaces Infantry after Grenadier research. Lobs grenades that scatter shrapnel — brutal against clusters. Slightly slower than standard infantry. 11s to train.',
    },
    { name:'MARKSMAN',     role:'SNIPER',     roles:['LONG RANGE','ANTI-INFANTRY','VISION'],
      hp:90,  spd:43,  atk:42,  rng:190, cost:220,
      color:'#CCFFCC', shape:'circle',
      lore:'Long-range sharpshooter with the best vision in the game (460px). Picks off infantry at extreme distance. Noticeably slower than standard troops and ineffective against armour (30% dmg). 14s to train.',
    },
    { name:'ANTI-TANK GUN',role:'ANTI-ARMOR', roles:['ANTI-ARMOR','HEAVY HIT'],
      hp:140, spd:43,  atk:72,  rng:120, cost:280,
      color:'#FF4400', shape:'square',
      lore:'Slow but devastating. Fires armour-piercing rounds that shred tank hulls. Weak against infantry. Requires Anti-Tank Gun research. 18s to train.',
    },
    { name:'SCOUT CAR',    role:'RECON',      roles:['RECON','FAST','VISION'],
      hp:45,  spd:135, atk:8,   rng:55,  cost:80,
      color:'#00FFCC', shape:'diamond',
      lore:'Fastest unit on the field. Reveals wide fog-of-war and flanks artillery. Fragile in a firefight — never send it alone. 5s to train.',
    },
    { name:'TANK',         role:'ARMOR',      roles:['FLANKING','ARMOR','ASSAULT'],
      hp:320, spd:34,  atk:46,  rng:100, cost:400,
      color:'#FFDD44', shape:'square',
      lore:'Main battle tank. Powerful cannon crushes anything it flanks. Vulnerable to AT guns from the front. Requires Tech Lab and War Factory. 20s to train.',
    },
    { name:'HEAVY TANK',   role:'BREAKTHROUGH',roles:['ARMORED','HIGH HP','ASSAULT'],
      hp:500, spd:28,  atk:60,  rng:100, cost:400,
      color:'#00AAFF', shape:'square',
      lore:'A rolling fortress. Takes enormous punishment and dishes it back. 1.9× damage vs infantry. Requires Heavy Tank research. 26s to train.',
    },
    { name:'ARTILLERY',    role:'SUPPORT',    roles:['LONG RANGE','SPLASH','DEPLOY'],
      hp:180, spd:28,  atk:85,  rng:280, cost:550,
      color:'#FFEE00', shape:'square',
      lore:'Must deploy before firing. Devastating splash damage at extreme range, helpless up close. Keep it behind your front line. Requires Artillery research. 28s to train.',
    },
    { name:'HARVESTER',    role:'LOGISTICS',  roles:['ECONOMY','UNARMED'],
      hp:200, spd:64,  atk:0,   rng:0,   cost:0,
      color:'#DDAA00', shape:'square',
      lore:'Collects Tiberium and converts it to credits at the Refinery. Auto-spawns when a Refinery is built; replacements cost 300¢. Protect it at all costs.',
    },
  ];

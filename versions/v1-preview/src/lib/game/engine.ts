...
export class Engine {
  ...
  // Tiberium eco
  _tibSpawnTimer: number = rnd(45, 70);  // seconds until next field spawns
  _tibSpawnCount: number = 0;            // extra fields spawned so far
  ...
  _init() {
    ...
    this._tibSpawnTimer = rnd(45, 70); this._tibSpawnCount = 0;
  ...
  placeBuild(cx: number, cy: number) {
    ...
    if (!this._isValidBuildPos(cx, cy)) {
      return this.setStatus('Cannot build here — too close to enemy or obstacle', 'error');
    }
    this._credits -= cost;
    const b = type === 'Turret'
  ...
  update(dt: number) {
    ...
    // Keep tib fields alive even when empty — they regen over time
    this._recalcPower();

    // ── TIBERIUM ECO ─────────────────────────────────────
    for (const f of this.tibFields) f.regen(dt);
    this._tibSpawnTimer -= dt;
    if (this._tibSpawnTimer <= 0 && this._tibSpawnCount < 5) {
      this._trySpawnTibField();
      this._tibSpawnTimer = rnd(40, 70);
    }
  ...
  _trySpawnTibField() {
    for (let attempt = 0; attempt < 30; attempt++) {
      const x = rnd(60, CANVAS_W - 60);
      const y = rnd(60, CANVAS_H - 60);
      // Avoid river (sinusoidal curve, approximate)
      const riverX = 448 + 30 * Math.sin(y * 0.009);
      if (Math.abs(x - riverX) < 65) continue;
      // Avoid buildings
      if (this.buildings.some(b => hypot(x, y, b.cx, b.cy) < 95)) continue;
      // Avoid existing tib fields
      if (this.tibFields.some(f => hypot(x, y, f.cx, f.cy) < 120)) continue;
      // Spawn
      const nf = new TiberiumField(x, y);
      this.tibFields.push(nf);
      this._ref.tibFields = this.tibFields;
      this._tibSpawnCount++;
      this._flashMsgs.push({
        text: 'TIBERIUM!', x, y: y - 38,
        t: 2.5, maxT: 2.5, color: C.tibGreen,
      });
      this.setStatus('New Tiberium field detected!', 'success');
      return;
    }
  }
  ...
  _isValidBuildPos(cx: number, cy: number): boolean {
    ...
    const bx = cx - d.w / 2, by = cy - d.h / 2;
    const margin = 6;

    // Canvas bounds
    if (bx < margin || by < margin ||
        bx + d.w > CANVAS_W - margin || by + d.h > CANVAS_H - margin) return false;

    // No overlap with existing buildings (+ clearance)
    for (const b of this.buildings) {
      if (bx < b.x + b.w + margin && bx + d.w + margin > b.x &&
          by < b.y + b.h + margin && by + d.h + margin > b.y) return false;
    }

    // Must not place within 110px of any enemy building
    for (const b of this.buildings) {
      if (b.team === 'enemy' && hypot(cx, cy, b.cx, b.cy) < 110) return false;
    }

    // Must not place in the river
    const riverX = 448 + 30 * Math.sin(cy * 0.009);
    if (cx > riverX - 52 && cx < riverX + 52) return false;
  ...
  draw(t: number, dragRect: { x:number;y:number;w:number;h:number } | null, mousePos: {x:number;y:number}) {
    ...
    // ── War-table holographic overlay ─────────────────────
    // Projection tint (green phosphor wash)
    ctx.fillStyle = 'rgba(0,12,6,0.13)';
    ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);

    // Animated scan line sweeping down
    const scanY = ((t * 55) % (CANVAS_H + 80)) - 40;
    const sg = ctx.createLinearGradient(0, scanY, 0, scanY + 60);
    sg.addColorStop(0,   'rgba(0,255,80,0)');
    sg.addColorStop(0.45,'rgba(0,255,80,0.055)');
    sg.addColorStop(1,   'rgba(0,255,80,0)');
    ctx.fillStyle = sg;
    ctx.fillRect(0, scanY, CANVAS_W, 60);
    ...
    // ── Vignette + corner markers (drawn last) ────────────
    const vig = ctx.createRadialGradient(
      CANVAS_W * 0.5, CANVAS_H * 0.5, CANVAS_H * 0.28,
      CANVAS_W * 0.5, CANVAS_H * 0.5, CANVAS_H * 0.82,
    );
    vig.addColorStop(0, 'rgba(0,0,0,0)');
    vig.addColorStop(1, 'rgba(0,5,2,0.64)');
    ctx.fillStyle = vig;
    ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
    ...
    const M = 22;
    ctx.lineWidth = 2;
    ctx.strokeStyle = 'rgba(0,238,85,0.9)';
    ctx.shadowBlur = 10; ctx.shadowColor = '#00EE55';
    for (const [cx, cy, dx, dy] of [
      [0, 0, 1, 1], [CANVAS_W, 0, -1, 1],
      [CANVAS_W, CANVAS_H, -1, -1], [0, CANVAS_H, 1, -1],
    ...
    // Build ghost — green if valid, red if blocked
    if (this._buildMode) {
      const d     = BDEF[this._buildMode];
      const mx    = mousePos.x - d.w / 2, my = mousePos.y - d.h / 2;
      const valid = this._isValidBuildPos(mousePos.x, mousePos.y);
      ctx.fillStyle   = valid ? 'rgba(80,150,255,0.22)' : 'rgba(255,50,50,0.22)';
      ctx.fillRect(mx, my, d.w, d.h);
      ctx.strokeStyle = valid ? C.allyLight : C.error;
      ctx.lineWidth   = 1.5;
      ctx.setLineDash([5, 5]);
      ctx.strokeRect(mx, my, d.w, d.h);
      ctx.setLineDash([]);
      // Exclusion radius ring when near enemy
      if (!valid) {
        ctx.beginPath();
      ...
      ctx.fillStyle = valid ? '#CCDDFF' : '#FFAAAA';
      ctx.font = '9px "Courier New"';
      ctx.textAlign = 'center';
      ctx.fillText(valid ? this._buildMode : '✗ BLOCKED', mousePos.x, mousePos.y + 3);
      ctx.textAlign = 'left';
    }
  }

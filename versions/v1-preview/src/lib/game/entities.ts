...
export class Building extends Entity {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    // Building glow — punchy enough to read through vignette
...
export class TiberiumField {
  ...
  remaining: number;
  radius:    number = 44;
  regenRate: number = 1.8;   // credits/s of passive regrowth when not being harvested
  private _crystals: Array<{ x:number; y:number; h:number; w:number; angle:number }> = [];

  constructor(cx: number, cy: number) {
    this.id       = nextId();
    this.cx       = cx; this.cy = cy;
    this.capacity = rnd(700, 1200);   // bigger piles
    this.remaining = this.capacity;
    const n = rndi(10, 18);           // more crystals per field
    for (let i = 0; i < n; i++) {
      this._crystals.push({
  ...
  regen(dt: number) {
    if (this.remaining < this.capacity) {
      this.remaining = Math.min(this.capacity, this.remaining + this.regenRate * dt);
    }
  }
...
export class Unit extends Entity {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    // Glow ring (faction colour) — boosted for war-table readability
    if (!this._flash) {
      ctx.shadowBlur  = 12;
      ctx.shadowColor = pl ? C.allyAccent : C.enemyAccent;
      ctx.beginPath(); ctx.arc(this.x, this.y, r + 3, 0, Math.PI * 2);
      ctx.fillStyle = pl ? 'rgba(68,170,255,0.22)' : 'rgba(255,85,34,0.22)';
      ctx.fill();
      ctx.shadowBlur = 0;
    }
...
export class Harvester extends Unit {
  ...
  update(dt: number, allUnits: Unit[], _proj: Projectile[]) {
    ...
    if (this.state === 'idle') {
      ...
      // Score fields by: richness / distance — prefer closer AND richer fields
      const MIN_VIABLE = 20;   // don't bother with nearly empty fields
      const field = g.tibFields
        .filter(f => f.remaining >= MIN_VIABLE)
        .sort((a, b) => {
          const scoreA = a.remaining / Math.max(1, hypot(this.x, this.y, a.cx, a.cy));
          const scoreB = b.remaining / Math.max(1, hypot(this.x, this.y, b.cx, b.cy));
    ...
    if (this.state === 'to-field') {
      if (!this.targetField || this.targetField.remaining < 5) {
